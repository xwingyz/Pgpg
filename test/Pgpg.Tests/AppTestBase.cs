...
namespace Pgpg.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PgpgDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PgpgTestModule>
    {
        ...
        protected void UsingDbContext(Action<PgpgDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PgpgDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PgpgDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PgpgDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PgpgDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PgpgDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PgpgDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PgpgDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PgpgDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PgpgDbContext, Task<T>> func)
        {
            T result;
